...
class NavigationSystem(esper.Processor):
    ...
    - Lecture : Transform + Velocity + Speed (+ TerrainEffect) + Path + PathProgress
    - Si Transform/Velocity/Speed manquent, on les ajoute automatiquement.
    - Accélération + freinage (arrive behavior simple).
    - Slow terrain :
        * Speed.mult_terrain (si un TerrainEffectSystem l'a déjà mis à jour)
        * TerrainEffect.apply() si le composant existe.
    """

    def __init__(
        self,
        *,
        max_accel: float = 8.0,        # unités monde / s^2
        arrive_radius: float = 0.05,   # distance pour "snap" au nœud
        min_speed: float = 0.0
    ):
        super().__init__()
    ...
    def process(self, dt: float):
        ...
        for ent, (gpos, path, prog) in esper.get_components(GridPosition, Path, PathProgress):
            # assure Transform/Velocity/Speed
            ...
            # nœud cible = prochain dans la liste
            ...
            # vitesse effective (base * mult terrain * terrain_effect optional)
            ...
            if esper.has_component(ent, TerrainEffect):
                ...
                # on suppose que TerrainEffect a une méthode apply(vitesse)
                eff_speed = terr.apply(eff_speed)

            # freinage : si on est proche du nœud, on réduit la vitesse désirée
            ...
            # on rapproche la vitesse actuelle vers la vitesse désirée (accélération capée)
